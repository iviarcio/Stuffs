...
//===----------------------------------------------------------------------===//
//
// NSP Sharding Propagation pass.
//
// This module is mostly from upstream Mesh/Transforms/ShardingPropagation.cpp
// The main difference is NSP-specific adjustement to how propagation boundaries
// are handled.
//
// In particular, this pass preserves the upstream propagatioon behavior for
// tensor-based operations, but treats bufferization.materialize_in_destination
// as a hard boundary. This prevents sharding from being propagated across a
// tensor-to-buffer transition where the destination is a memref.
//
// This behavior is necessary because propagating sharding through such a sink
// violates LLVM/MLIR ShardingInterface constraints and would be rejected
// during verification.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "nsp-sharding-propagation"
...
static LogicalResult visitOp(Operation *op, OpBuilder &builder) {
  // bufferization.materialize_in_destination is a terminal tensor-to-buffer
  // sink in the NSP pipeline. It may consume a tensor value that carries shard
  // annotations, but its destination operand is a memref and must not
  // participate in shard propagation.
  //
  // Newer LLVM/MLIR versions validate ShardingInterface implementations more
  // strictly and reject non-tensor/non-scalar operands. Keep this op as an NSP
  // boundary and let NSPLocalize/NSPMaterialize handle it explicitly.
...
//===----------------------------------------------------------------------===//
// NSPShardingPropagation
//===----------------------------------------------------------------------===//
struct NSPShardingPropagationPass
    : public PassWrapper<NSPShardingPropagationPass,
                         OperationPass<func::FuncOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(NSPShardingPropagationPass)

  StringRef getArgument() const final { return "nsp-sharding-propagation"; }

  StringRef getDescription() const final {
    return "NSP-specific sharding propagation with tensor-to-buffer sink boundaries";
  }
  ...
  void runOnOperation() override {
    ...
    // Keep the NSP wrapper deterministic and close to the upstream default
    // behavior used by this pipeline: run a backward pass first so result-side
    // annotations can drive operands, then run a forward pass so operand-side
    // annotations can drive results.
    if (traverse(llvm::reverse(block), builder, "backward"))
      return;

    if (traverse(block, builder, "forward"))
      return;
  }
};
...
std::unique_ptr<Pass> createNSPShardingPropagationPass() {
  return std::make_unique<NSPShardingPropagationPass>();
}
