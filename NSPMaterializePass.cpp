...
materializeRank2InputAsSubview(OpBuilder &b, Location loc, Value inputTensor) {
  ...
  if (auto allSlice = base.getDefiningOp<mlir::shard::AllSliceOp>()) {
    ...
    Value tileRows =
        arith::ConstantIndexOp::create(b, loc, resultTy.getShape()[0]);
    Value off0 = arith::MulIOp::create(b, loc, *linearIdxOrFail, tileRows);
    ...
    if (auto allSliceOp = baseTensor.getDefiningOp<mlir::shard::AllSliceOp>()) {
      ...
      if (offsets.empty()) {
        ...
        if (auto old0 = dyn_cast<Value>(offsets[0])) {
          Value newOff0 = arith::AddIOp::create(b, loc, off0, old0).getResult();
          offsets[0] = newOff0;
        } else if (auto old0Attr = dyn_cast<Attribute>(offsets[0])) {
          Value old0Val = arith::ConstantIndexOp::create(
                              b, loc, cast<IntegerAttr>(old0Attr).getInt())
                              .getResult();
          Value newOff0 =
              arith::AddIOp::create(b, loc, off0, old0Val).getResult();
          offsets[0] = newOff0;
        } else {
          return failure();
        }
...
materializeTileToDestination(OpBuilder &b,
  ...
  Value tileExtent0 = arith::ConstantIndexOp::create(b, loc, tileShape[0]);
  Value offset0 = arith::MulIOp::create(b, loc, procIdx, tileExtent0);
