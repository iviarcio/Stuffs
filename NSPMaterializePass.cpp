...
static SmallVector<int16_t, 2> getGridAxesFromOp(Operation *op) {
  ...
  Attribute attr = op->getAttr("grid_axes");
  if (!attr)
    return axes;

  if (auto dense = dyn_cast<DenseI16ArrayAttr>(attr)) {
    for (int16_t axis : dense.asArrayRef())
      axes.push_back(axis);
    return axes;
  }

  if (auto dense = dyn_cast<DenseI64ArrayAttr>(attr)) {
    for (int64_t axis : dense.asArrayRef())
      axes.push_back(static_cast<int16_t>(axis));
    return axes;
  }
  ...
  if (auto array = dyn_cast<ArrayAttr>(attr)) {
    for (Attribute elem : array) {
      if (auto intAttr = dyn_cast<IntegerAttr>(elem))
        axes.push_back(static_cast<int16_t>(intAttr.getInt()));
    }
  }
...
static bool
isSimpleLocalizedElementwiseGeneric(linalg::GenericOp g,
...
/// directly on memrefs.  When it succeeds, the old tensor producer chain must
/// be removed explicitly; otherwise allocation/copy-like ops such as
...
static bool isDeadTensorProducerSafeToErase(Operation *op) {
  ...
  if (isa<tensor::CastOp, tensor::CollapseShapeOp, tensor::ExpandShapeOp,
          tensor::EmptyOp, tensor::ExtractSliceOp, bufferization::AllocTensorOp,
          bufferization::ToTensorOp, mlir::shard::AllSliceOp>(op))
    return true;
...
static void
eraseDeadTensorProducerChain(Value value,
...
materializeTileToDestination(OpBuilder &b,
  ...
  auto getGridAxesForMaterializedDim = [&](int64_t dim)
      -> FailureOr<SmallVector<int16_t, 2>> {
    auto destDim = destTy.getDimSize(dim);
    if (ShapedType::isDynamic(destDim) || destDim == tileShape[dim])
      return SmallVector<int16_t, 2>{};

    if (tileShape[dim] <= 0 || destDim <= 0 || destDim % tileShape[dim] != 0)
      return failure();

    int64_t factor = destDim / tileShape[dim];

    if (gridShape.size() == 2) {
      if (factor == gridShape[0])
        return SmallVector<int16_t, 2>{0};
      if (factor == gridShape[1])
        return SmallVector<int16_t, 2>{1};
      if (factor == gridShape[0] * gridShape[1])
        return SmallVector<int16_t, 2>{0, 1};
    }

    if (gridShape.size() == 1 && factor == gridShape[0])
      return SmallVector<int16_t, 2>{0};
  ...
  for (int64_t d = 0; d < rank; ++d) {
    FailureOr<SmallVector<int16_t, 2>> axesOr =
        getGridAxesForMaterializedDim(d);
    if (failed(axesOr))
      return failure();

    SmallVector<int16_t, 2> axes = *axesOr;
    if (axes.empty()) {
      ...
      Value tileOffset = arith::MulIOp::create(b, loc, *procIdxOr, tileExtent);
      offsets.push_back(OpFoldResult(tileOffset));
    }
  ...
  // Legacy fallback: if the destination shape did not expose any sharded
  // dimension but split_axis is present, preserve the old single-axis behavior.
  bool hasNonZeroDynamicOffset = false;
  for (OpFoldResult ofr : offsets) {
    if (isa<Value>(ofr)) {
      hasNonZeroDynamicOffset = true;
      break;
    }
    auto attr = dyn_cast<Attribute>(ofr);
    auto intAttr = attr ? dyn_cast<IntegerAttr>(attr) : IntegerAttr();
  ...
  if (!hasNonZeroDynamicOffset) {
    ...
    Value tileOffset = arith::MulIOp::create(b, loc, *procIdxOr, tileExtent);
    offsets[splitAxis] = OpFoldResult(tileOffset);
  }
