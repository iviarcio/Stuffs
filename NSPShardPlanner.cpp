...
private:
  ...
  static FailureOr<ShardPlan> buildPlanForGeneric(linalg::GenericOp op,
    ...
    if (auto transposeChoice = pickShardIteratorFromTransposeInput(op))
      ...
      plan.shardIter = pickShardIteratorIndex(outMap, iters, policy, outTy);
    if (plan.shardIter < 0)
      return failure();
  ...
  pickShardIteratorIndex(AffineMap outMap,
                         ...
                         const ShardPolicy &policy, RankedTensorType outputTy) {
    ...
    if (outputTy && outputTy.hasStaticShape()) {
      for (Candidate candidate : candidates) {
        ...
        if (extent > 0 && policy.nspCount > 0 && extent % policy.nspCount == 0)
