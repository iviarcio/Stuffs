...
getUnitCollapseRepresentativeDims(tensor::CollapseShapeOp collapse) {
  ...
  auto resultTy =
      dyn_cast<RankedTensorType>(collapse->getResult(0).getType());
  if (!srcTy || !resultTy)
    return std::nullopt;
...
struct ExpandShapeShardingModel
    : public shard::ShardingInterface::ExternalModel<
          ExpandShapeShardingModel, tensor::ExpandShapeOp> {
  ...
  getShardingAnnotations(Operation *op,
    ...
    auto inputArr =
        toFixedRankShardingArray(inputSharding, srcTy.getRank());
    auto resultArr = expandInputShardingToExpandShapeResult(
        inputArr, *representatives, resultTy.getRank());
    auto resultOpt = makeShardingOptionFromAxes(
        std::move(resultArr), inputSharding.getGridAttr());
...
struct CollapseShapeShardingModel
    : public shard::ShardingInterface::ExternalModel<
          CollapseShapeShardingModel, tensor::CollapseShapeOp> {
