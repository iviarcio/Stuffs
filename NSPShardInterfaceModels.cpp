...
//===----------------------------------------------------------------------===//
// tensor.expand_shape / tensor.collapse_shape helpers
//===----------------------------------------------------------------------===//

/// Return, for each source dimension, the representative result dimension that
/// should carry the sharding.
///
/// This only supports the conservative unit-expansion case, such as:
///   tensor<M> -> tensor<M x 1>
///
/// In that form, the original dimension maps to the only non-unit dimension in
/// its reassociation group. Newly inserted unit dimensions remain replicated.
///
/// General reshapes are intentionally not modeled here, because the mapping is
/// not a simple affine/view-like sharding projection.
...
getUnitExpandRepresentativeDims(tensor::ExpandShapeOp expand) {
  ...
  for (ArrayRef<int64_t> group : reassociation) {
    ...
    for (int64_t resultDim : group) {
      ...
      // Dynamic dimensions inside a multi-dim reassociation group cannot be
      // proven to be unit expansions. Keep the model conservative.
    ...
    // All expanded dimensions are unit dimensions. This is degenerate, but
    // still view-like; pick the first result dimension as the representative.
    if (representative < 0)
...
static shard::ShardingArray expandInputShardingToExpandShapeResult(
    ArrayRef<SmallVector<shard::GridAxis>> input,
    ArrayRef<int64_t> representatives, int64_t resultRank) {
  shard::ShardingArray result;
  result.resize(resultRank);

  for (auto [srcDim, resultDim] : llvm::enumerate(representatives)) {
    if ((int64_t)srcDim < (int64_t)input.size() && resultDim < resultRank)
      result[resultDim] = input[srcDim];
  }

  return result;
}

static shard::ShardingArray collapseExpandShapeResultShardingToInput(
    ArrayRef<SmallVector<shard::GridAxis>> result,
    ArrayRef<int64_t> representatives, int64_t inputRank) {
  shard::ShardingArray input;
  input.resize(inputRank);

  for (auto [srcDim, resultDim] : llvm::enumerate(representatives)) {
    if ((int64_t)srcDim < inputRank && resultDim < (int64_t)result.size())
      input[srcDim] = result[resultDim];
  }

  return input;
}

/// Return, for each result dimension of tensor.collapse_shape, the source
/// dimension that should carry the sharding.
///
/// This only supports conservative unit-collapse cases, such as:
///   tensor<1xM> -> tensor<M>
///
/// In that form, the collapsed result dimension maps to the only non-unit
/// dimension in its reassociation group. Collapsed unit dimensions remain
/// replicated.
///
/// General reshapes are intentionally not modeled here, because the mapping is
/// not a simple affine/view-like sharding projection.
static std::optional<SmallVector<int64_t>>
getUnitCollapseRepresentativeDims(tensor::CollapseShapeOp collapse) {
  auto srcTy = dyn_cast<RankedTensorType>(collapse->getOperand(0).getType());
  auto resultTy = dyn_cast<RankedTensorType>(collapse->getResult(0).getType());
  if (!srcTy || !resultTy)
    return std::nullopt;
  ...
  if ((int64_t)reassociation.size() != resultTy.getRank())
    return std::nullopt;

  SmallVector<int64_t> representatives;
  representatives.reserve(resultTy.getRank());
  ...
  for (ArrayRef<int64_t> group : reassociation) {
    ...
    for (int64_t sourceDim : group) {
      if (sourceDim < 0 || sourceDim >= srcTy.getRank())
        return std::nullopt;

      int64_t dimSize = srcTy.getDimSize(sourceDim);

      // Dynamic dimensions inside a multi-dim reassociation group cannot be
      // proven to be unit collapses. Keep the model conservative.
      if (ShapedType::isDynamic(dimSize)) {
        if (group.size() != 1)
          return std::nullopt;
        representative = sourceDim;
        ++nonUnitDims;
        continue;
      }
    ...
    // All collapsed dimensions are unit dimensions. This is degenerate, but
    // still view-like; pick the first source dimension as the representative.
    if (representative < 0)
      representative = group.front();
...
static shard::ShardingArray collapseSourceShardingToCollapseShapeResult(
    ArrayRef<SmallVector<shard::GridAxis>> source,
    ArrayRef<int64_t> representatives, int64_t resultRank) {
  shard::ShardingArray result;
  result.resize(resultRank);

  for (auto [resultDim, sourceDim] : llvm::enumerate(representatives)) {
    if ((int64_t)resultDim < resultRank && sourceDim < (int64_t)source.size())
      result[resultDim] = source[sourceDim];
  }

  return result;
}

static shard::ShardingArray expandCollapseShapeResultShardingToSource(
    ArrayRef<SmallVector<shard::GridAxis>> result,
    ArrayRef<int64_t> representatives, int64_t sourceRank) {
  shard::ShardingArray source;
  source.resize(sourceRank);

  for (auto [resultDim, sourceDim] : llvm::enumerate(representatives)) {
    if (sourceDim < sourceRank && (int64_t)resultDim < (int64_t)result.size())
      source[sourceDim] = result[resultDim];
  }
...
struct ExpandShapeShardingModel
    : public shard::ShardingInterface::ExternalModel<ExpandShapeShardingModel,
  ...
  SmallVector<AffineMap> getIndexingMaps(Operation *op) const {
    ...
    inputExprs.reserve(representatives->size());
    for (int64_t resultDim : *representatives)
      inputExprs.push_back(getAffineDimExpr(resultDim, ctx));

    SmallVector<AffineExpr> resultExprs;
    resultExprs.reserve(resultRank);
    for (int64_t dim = 0; dim < resultRank; ++dim)
      resultExprs.push_back(getAffineDimExpr(dim, ctx));

    return {
        AffineMap::get(resultRank, 0, inputExprs, ctx),
        AffineMap::get(resultRank, 0, resultExprs, ctx),
    };
  }
  ...
  SmallVector<utils::IteratorType> getLoopIteratorTypes(Operation *op) const {
    ...
    if (!resultTy)
      return {};
    return makeParallelIters(resultTy.getRank());
  }
  ...
  getShardingOption(Operation *op, ArrayRef<shard::Sharding> operandShardings,
    ...
    // Canonicalize the option as input-rank sharding.
    if (!operandShardings.empty() && operandShardings[0])
      return makeValueShardingOption(operandShardings[0]);
    ...
    if (!resultShardings.empty() && resultShardings[0]) {
      ...
      auto inputArr = collapseExpandShapeResultShardingToInput(
          resultArr, *representatives, srcTy.getRank());
      return makeShardingOptionFromAxes(std::move(inputArr),
  ...
  getShardingAnnotations(Operation *op,
    ...
    std::vector<shard::Sharding> res;
    res.resize(2, shard::Sharding());

    shard::Sharding inputSharding =
        fromShardingOption(op, opt, srcTy.getRank());
    res[0] = inputSharding;

    auto inputArr = toFixedRankShardingArray(inputSharding, srcTy.getRank());
    auto resultArr = expandInputShardingToExpandShapeResult(
        inputArr, *representatives, resultTy.getRank());
    auto resultOpt = makeShardingOptionFromAxes(std::move(resultArr),
                                                inputSharding.getGridAttr());

    res[1] = succeeded(resultOpt)
                 ? fromShardingOption(op, *resultOpt, resultTy.getRank())
                 : shard::Sharding();
...
struct CollapseShapeShardingModel
    : public shard::ShardingInterface::ExternalModel<CollapseShapeShardingModel,
                                                     tensor::CollapseShapeOp> {

  SmallVector<AffineMap> getIndexingMaps(Operation *op) const {
    auto collapse = cast<tensor::CollapseShapeOp>(op);
    auto srcTy = dyn_cast<RankedTensorType>(op->getOperand(0).getType());
    if (!srcTy)
      return {};

    auto representatives = getUnitCollapseRepresentativeDims(collapse);
    if (!representatives)
      return {};

    MLIRContext *ctx = op->getContext();
    int64_t sourceRank = srcTy.getRank();

    SmallVector<AffineExpr> sourceExprs;
    sourceExprs.reserve(sourceRank);
    for (int64_t dim = 0; dim < sourceRank; ++dim)
      sourceExprs.push_back(getAffineDimExpr(dim, ctx));

    SmallVector<AffineExpr> resultExprs;
    resultExprs.reserve(representatives->size());
    for (int64_t sourceDim : *representatives)
      resultExprs.push_back(getAffineDimExpr(sourceDim, ctx));

    return {
        AffineMap::get(sourceRank, 0, sourceExprs, ctx),
        AffineMap::get(sourceRank, 0, resultExprs, ctx),
    };
  }
  ...
  SmallVector<utils::IteratorType> getLoopIteratorTypes(Operation *op) const {
    ...
    if (!srcTy)
      return {};
    return makeParallelIters(srcTy.getRank());
  }
  ...
  getShardingOption(Operation *op, ArrayRef<shard::Sharding> operandShardings,
    ...
    // Canonicalize the option as source-rank sharding.
    if (!operandShardings.empty() && operandShardings[0])
      return makeValueShardingOption(operandShardings[0]);
    ...
    if (!resultShardings.empty() && resultShardings[0]) {
      auto resultArr =
          toFixedRankShardingArray(resultShardings[0], resultTy.getRank());
      auto sourceArr = expandCollapseShapeResultShardingToSource(
          resultArr, *representatives, srcTy.getRank());
      return makeShardingOptionFromAxes(std::move(sourceArr),
                                        resultShardings[0].getGridAttr());
    }
  ...
  getShardingAnnotations(Operation *op,
    ...
    shard::Sharding sourceSharding =
        fromShardingOption(op, opt, srcTy.getRank());
    res[0] = sourceSharding;

    auto sourceArr = toFixedRankShardingArray(sourceSharding, srcTy.getRank());
    auto resultArr = collapseSourceShardingToCollapseShapeResult(
        sourceArr, *representatives, resultTy.getRank());
    auto resultOpt = makeShardingOptionFromAxes(std::move(resultArr),
                                                sourceSharding.getGridAttr());
...
void registerNSPShardInterfaceModels(DialectRegistry &registry) {
  ...
  registry.addExtension(+[](MLIRContext *ctx, memref::MemRefDialect *dialect) {
    ...
    // View-like memref op: sharding-transparent (dynamic sizes/offsets make
    // a precise model complex; treat as opaque boundary for propagation).
    memref::SubViewOp::attachInterface<NoShardingModel<memref::SubViewOp>>(
        *ctx);
  ...
  registry.addExtension(
      +[](MLIRContext *ctx, bufferization::BufferizationDialect *dialect) {
        ...
        bufferization::MaterializeInDestinationOp::attachInterface<
            NoShardingModel<bufferization::MaterializeInDestinationOp>>(*ctx);
      });

  // Tensor view-like ops.
  registry.addExtension(+[](MLIRContext *ctx, tensor::TensorDialect *dialect) {
    (void)dialect;
    tensor::ExpandShapeOp::attachInterface<ExpandShapeShardingModel>(*ctx);
    tensor::CollapseShapeOp::attachInterface<CollapseShapeShardingModel>(*ctx);
    // Slice / empty: sharding-transparent for propagation purposes.
    // extract_slice produces a ranked tensor but its offset/size/stride
    // operands are scalars; treat as a propagation boundary (no-op model).
    tensor::ExtractSliceOp::attachInterface<
        NoShardingModel<tensor::ExtractSliceOp>>(*ctx);
    tensor::EmptyOp::attachInterface<ResultOnlyShardingModel<tensor::EmptyOp>>(
        *ctx);
  });
  ...
  registry.addExtension(+[](MLIRContext *ctx, scf::SCFDialect *dialect) {
    ...
    scf::IfOp::attachInterface<NoShardingModel<scf::IfOp>>(*ctx);
  });
  ...
  registry.addExtension(+[](MLIRContext *ctx, arith::ArithDialect *dialect) {
    ...
    // Scalar integer comparison /  min / max / bit ops
    arith::CmpIOp::attachInterface<NoShardingModel<arith::CmpIOp>>(*ctx);
    arith::CmpFOp::attachInterface<NoShardingModel<arith::CmpFOp>>(*ctx);
    arith::MinSIOp::attachInterface<NoShardingModel<arith::MinSIOp>>(*ctx);
    arith::MaxSIOp::attachInterface<NoShardingModel<arith::MaxSIOp>>(*ctx);
    arith::MinUIOp::attachInterface<NoShardingModel<arith::MinUIOp>>(*ctx);
    arith::MaxUIOp::attachInterface<NoShardingModel<arith::MaxUIOp>>(*ctx);
    arith::AndIOp::attachInterface<NoShardingModel<arith::AndIOp>>(*ctx);
    arith::OrIOp::attachInterface<NoShardingModel<arith::OrIOp>>(*ctx);
    arith::XOrIOp::attachInterface<NoShardingModel<arith::XOrIOp>>(*ctx);
    arith::SelectOp::attachInterface<NoShardingModel<arith::SelectOp>>(*ctx);
    arith::ExtSIOp::attachInterface<NoShardingModel<arith::ExtSIOp>>(*ctx);
    arith::ExtUIOp::attachInterface<NoShardingModel<arith::ExtUIOp>>(*ctx);
    arith::TruncIOp::attachInterface<NoShardingModel<arith::TruncIOp>>(*ctx);
    arith::TruncFOp::attachInterface<NoShardingModel<arith::TruncFOp>>(*ctx);
    arith::SIToFPOp::attachInterface<NoShardingModel<arith::SIToFPOp>>(*ctx);
    arith::FPToSIOp::attachInterface<NoShardingModel<arith::FPToSIOp>>(*ctx);
    arith::UIToFPOp::attachInterface<NoShardingModel<arith::UIToFPOp>>(*ctx);
    arith::FPToUIOp::attachInterface<NoShardingModel<arith::FPToUIOp>>(*ctx);
    arith::ShLIOp::attachInterface<NoShardingModel<arith::ShLIOp>>(*ctx);
    arith::ShRSIOp::attachInterface<NoShardingModel<arith::ShRSIOp>>(*ctx);
    arith::ShRUIOp::attachInterface<NoShardingModel<arith::ShRUIOp>>(*ctx);
    arith::RemSIOp::attachInterface<NoShardingModel<arith::RemSIOp>>(*ctx);
    arith::RemUIOp::attachInterface<NoShardingModel<arith::RemUIOp>>(*ctx);
    arith::DivSIOp::attachInterface<NoShardingModel<arith::DivSIOp>>(*ctx);
    arith::DivUIOp::attachInterface<NoShardingModel<arith::DivUIOp>>(*ctx);
    arith::NegFOp::attachInterface<NoShardingModel<arith::NegFOp>>(*ctx);
    arith::ExtFOp::attachInterface<NoShardingModel<arith::ExtFOp>>(*ctx);
